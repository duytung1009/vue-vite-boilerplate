<template>
  <BaseTemplate title="Vite-TS-Tailwind Starter">
    ...
    <ul>
      ...
      <li>Github Workflows and Gitlab CI preconfigured to run e2e and component tests automatically on every push</li>
    </ul>
    ...
    <p>
      ...
      <a href="https://v3.vuejs.org/api/sfc-tooling.html#ide-support">official IDE Support documentation</a>
      for more details.
    </p>
    ...
    <p class="text-gray-500">Note: the development server needs to be running when using the Cypress test runner.</p>
    ...
    <p>
      See
      <code>package.json</code> for all available commands.
    </p>
    ...
    <p>
      If you need more than one parameter in the typed Vuex actions, use `payload` object instead of multiple
...
[Action.updateItem](
  ...
  payload: {
    ...
    <p>
      ...
      <code>meta.env</code> object for ENV variables (see
    ...
    <p>
      See
      <code>vite.config.js</code> and <code>src/env.d.ts</code> for the configuration and
    ...
    <p>
      ...
      <code>tailwind.config.js</code> has a custom plugin that allows you to use a <strong>'firefox' variant</strong> in
      your Tailwind markup to target FireFox. The setup uses Tailwind JIT mode and PurgeCSS so this doesn't add any
      weight to your CSS if you don't use it. But, it's there if you happen to need it!
    </p>
...
&lt;/div&gt;</pre
    ...
    <p>
      TL;DR: due to Vite and esbuild design decisions and ongoing work in this area, code coverage is not yet added to
      this template.
    </p>
    ...
    <p>
      ...
      <a href="https://gitlab.com/uninen/vue-cli-template">https://gitlab.com/uninen/vue-cli-template</a>. That's a real
      world example of proper coverage configuration that works automatically in CI. But. This setup doesn't work with
      Vite.
    </p>

    <p>
      Typical Vite pipelines doesn't use babel at all which is needed above for automatically instrument the
      transpilated code. Vite is powered by eslint which has decided
      <a href="https://github.com/evanw/esbuild/issues/184">code coverage being out of scope</a>. There's a
      <a href="https://github.com/vitejs/vite/issues/1955">ticket tracking proper Jest integration w/ Vite</a>
      which would probably sort code coverage as well. For now, you should either build whatever custom pipeline you
      need manually. If you have a smart solution for this, PRs welcome!
    </p>
